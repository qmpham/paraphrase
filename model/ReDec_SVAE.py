...
class ReDec_SVAE:
    ...
    def _compute_loss(self, outputs, tgt_ids_batch, tgt_length, params, mode, mu_states, logvar_states):
        ...
        if self.Loss_type == "Cross_Entropy":
            ...
            loss_2, loss_normalizer_2, loss_token_normalizer_2 = cross_entropy_sequence_loss(
                logits_2,
                tgt_ids_batch, 
                tgt_length + 1,                                                         
                label_smoothing = params.get("label_smoothing", 0.0),
            ...
            loss = (loss + loss_2) / 2
            loss_normalizer = (loss_normalizer + loss_normalizer_2) / 2
            loss_token_normalizer = (loss_token_normalizer + loss_token_normalizer_2) / 2
...
#==============================DECODER2==================================
            decoder_2 = onmt.decoders.AttentionalRNNDecoder(
                nlayers, 
                num_units=hidden_size, 
                bridge=onmt.layers.CopyBridge(), 
                attention_mechanism_class=tf.contrib.seq2seq.LuongAttention,
        ...
        if mode =="Training":            
            ...
            with tf.variable_scope("encoder", reuse=tf.AUTO_REUSE):
                ...
                if config["Architecture"] == "Transformer":
                    ...
                    encoder_states_combined = tf.concat([encoder_states_src[-1][0], encoder_states_src[-1][1]], -1)
                        # dim = [batch_size, hidden_size + hidden_size]
            ...
            with tf.variable_scope("generator"):
                ...
                if config["Architecture"] == "Transformer":
                    ...
                    emb_tgt_batch_in = tf.reshape(emb_tgt_batch_in, [batch_size, -1, size_src + latent_variable_size])
            ...
            with tf.variable_scope("decoder1", reuse=tf.AUTO_REUSE): 
                ...
                if config["Architecture"] == "LSTM":
                    ...
                    ids_logits = tf.argmax(logits,-1)[:,:-1]
                    
                    emb_batch_1 = tf.nn.embedding_lookup(src_emb, ids_logits)
            ...
            with tf.variable_scope("decoder2"): 
                ...
                if config["Architecture"] == "LSTM":
                    ...
                    logits_2, _, _, _ = decoder_2.decode(
                                              emb_tgt_batch_in, 
                                              tgt_length + 1,
                                              vocab_size = int(self.tgt_vocab_size),
                                              initial_state = new_encoder_states_src,
                                              ...
                                              mode = tf.estimator.ModeKeys.TRAIN,
                                              memory = emb_batch_1,
                                              memory_sequence_length = dec_length - 1,
                                              return_alignment_history = True
                    ) 
...
#-----------------------------------------------------INFERENCE MODE-----------------------------------------------#
           ...
           #========ENCODER_PROCESS======================
            ...
            with tf.variable_scope("generator"):
                ...
                if config.get("Inference from normal", True) :
            ...
            with tf.variable_scope("decoder1", reuse=tf.AUTO_REUSE):        
                ...
                # with tf.variable_scope("decoder1", ): 
                    ...
                    # dim sampled_ids : [batch, 1, maxlen] (beam = 1)

                    # print(sampled_length_1)
                    

                    sampled_ids_1 = tf.reshape(sampled_ids_1, [batch_size,-1])
                    sampled_length_1 = tf.reshape(sampled_length_1, [batch_size])
#                     start_ids = tf.fill([batch_size,1], constants.START_OF_SENTENCE_ID)

#                     emb_batch_in_for_dec2 = tf.nn.embedding_lookup(tgt_emb,tf.concat([start_ids, sampled_ids_1], 1))
                    
#                     print("emb_batch_in_for_dec2:",emb_batch_in_for_dec2)
                    
#                     print("z:",z)

#                     zzz = tf.tile(tf.reshape(z, [batch_size,1,-1]),[1, tf.shape(emb_batch_in_for_dec2)[1], 1])
                    
#                     print("zzz:",zzz)

#                     emb_batch_in_for_dec2 = tf.concat([emb_batch_in_for_dec2, zzz],2)

#                     emb_batch_in_for_dec2 = tf.reshape(emb_batch_in_for_dec2, [batch_size, -1, size_src + latent_variable_size])

#                     print("emb_batch_in_for_dec2_final:",emb_batch_in_for_dec2)

                    emb_batch_1 = tf.nn.embedding_lookup(src_emb, sampled_ids_1)

#             with tf.variable_scope("decoder1", reuse=True): 
#                     logits, dec_states, dec_length, _ = decoder.decode(
#                                           emb_batch_in_for_dec2, 
#                                           sampled_length_1 + 1,
#                                           vocab_size = int(self.tgt_vocab_size),
#                                           initial_state = encoder_states_src,
#                                           output_layer = output_layer,                                              
...
#                     print("logits:",logits)
            ...
            with tf.variable_scope("decoder2"): 
                ...
                if config["Architecture"] == "Transformer":
                    ...
                    if beam_width <= 1:                
                        sampled_ids, _, sampled_length, log_probs, alignment = decoder_2.dynamic_decode(
                                                                        lambda ids: tf.concat([tf.nn.embedding_lookup(tgt_emb,ids),zz],1),
                                                                        start_tokens,
                                                                        end_token,
                                                                        vocab_size=int(self.tgt_vocab_size),
                                                                        ...
                                                                        mode=tf.estimator.ModeKeys.PREDICT,
                                                                        memory=emb_batch_1,
                                                                        memory_sequence_length=sampled_length_1,
                                                                        dtype=tf.float32,
                                                                        return_alignment_history=True
                                                                        )
                    else:
                        length_penalty = config.get("length_penalty", 0)
                        sampled_ids, _, sampled_length, log_probs, alignment = decoder_2.dynamic_decode_and_search(
                                                              lambda ids: tf.concat([tf.nn.embedding_lookup(tgt_emb,ids),zz],1),
                                                              start_tokens,
                                                              end_token,
                                                              vocab_size = int(self.tgt_vocab_size),
        ...
        return outputs, (predictions_1, predictions), tgt_ids_batch, tgt_length, mu, logvar         